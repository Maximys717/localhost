...
$result5 = mysql_query("SELECT subscriber_id,the_followed_id FROM readers WHERE the_followed_id='$got_id'",$db); 
...
// если подписка сделана данным пользователем, то разрешаем еЄ удалить
if ($got_id == $myrow5['subscriber_id'] OR $got_id == $myrow5['the_followed_id']) {
	...
	$got_login = $myrow4['login'];
	exit("ѕользовател€ $got_login пока никто не читает! <html><head><meta http-equiv='Refresh' content='5; URL=page.php?id=".$got_id."'></head><body>¬ы будете перемещены через 5 сек. ≈сли не хотите ждать, то <a href='page.php?id=".$got_id."'>нажмите сюда.</a></body></html>");
	}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
<html>
    <head>
        <title>—писок читателей "<?php echo $got_login; ?>"</title>
    </head>
    <body>
		<?php print <<<HERE
<!--// выводим меню -->
...
HERE;
		?>
    <h2>—писок читателей "<?php print ("<a href='page.php?id=$got_id'>$got_login</a>"); ?>"</h2>
...
$myrow7 = mysql_fetch_array($result7);
	...
	// выводим их в цикле
		...
		printf("<a href='page.php?id=%s'>%s</a><br>",$myrow7['subscriber_id'],$myrow7['login']);
		}
